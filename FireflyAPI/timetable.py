...
class Timetable(DiscretelyAuthenticatedObject):
    ...
    This object can either be created via the AuthenticatedUser class or by passing the auth_blob value upon creation.
    Args:
    ...
    def get_lessons(self, start, end):
        ...
        data = {
            "data": "query Query { events(start: \"" + start_day + "Z\", for_guid: \"" +
                    self._DiscretelyAuthenticatedObject__guid + "\", end: \"" +
                    end_day + "Z\"){end,location,start,subject,description,guid,attendees{role,principal{guid,name}}}}"}
        response = requests.post(self._DiscretelyAuthenticatedObject__portal + "/_api/1.0/graphql",
