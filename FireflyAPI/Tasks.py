...
class TaskInterface(DiscretelyAuthenticatedObject):
    ...
    def getTasks(self, filter = TaskInterfaceFilter("Todo")):
        ...
        session = Utils.getSessionToken(self._DiscretelyAuthenticatedObject__auth_blob)
        cookies = {"ASP.NET_SessionId": session}
        response = requests.post(self._DiscretelyAuthenticatedObject__portal + "/api/v2/taskListing/view/student/tasks/all/filterBy",
    ...
    def __getTasksFromIds(self, ids):
        ...
        json_data = {"ids":ids}
        response = requests.post(self._DiscretelyAuthenticatedObject__portal + "/api/v2/apps/tasks/byIds", params=params, data=json_data)
...
class Task(DiscretelyAuthenticatedObject):
    ...
    def __load_task_details(self, task_data):
        self.event_store = TaskEvents.TaskEventStore(task_data["recipientsResponses"][0]["responses"], self._DiscretelyAuthenticatedObject__guid)
        ...
        if task_data["descriptionDetails"] != None:
        ...
        if task_data["fileAttachments"] != None:
    ...
    def __setCompletionStatus(self, done):
        ...
        params = {"ffauth_device_id": self._DiscretelyAuthenticatedObject__device_id,
                  "ffauth_secret": self._DiscretelyAuthenticatedObject__device_token}
        data = {"data": str({"event":{"type":completion_mapping[done],"assessment_details_id":0},"recipient":{"guid":self._DiscretelyAuthenticatedObject__guid,"type":"user"}})}
        response = requests.post(self._DiscretelyAuthenticatedObject__portal + "/_api/1.0/tasks/"+str(self.id)+"/responses", params=params, data=data)
